...
### ORM 엔터티 및 열 조회
 ...
 ORM `Session.execute()` 메서드를 이용해 쿼리문을 실행 시킬 때 `User` 클래스와 같은 전체 엔티티를 사용하면 결과 세트에서 
...
또는 엔티티 속성을 이용하여 `row` 내의 ORM 엔티티 컬럼을 가져올 수 있습니다.
...
보통 `Select`객체나 `select()` 생성자를 이용 할 때는 컬럼을 조회하는 경우가 다수지만 가끔은 임의로 스트링 문자열과 함께 컬럼을 조회를 해야하는 경우가 있습니다. 이번 섹션에서는 스트링 데이터를 조회 하는 방법에 다룹니다.
...
`column`을 표시하고 서브쿼리를 이용 할 수 있게 라벨 기능을 제공합니다.
...
첫 째는 `Select.join_from()`로 
JOIN구문에서 명시적으로 왼쪽테이블, 오른쪽 테이블을 지정할 수 있게 합니다.
두 번째는  `Select.join()`로 오직 오른쪽 테이블만 명시되어 있다.
...
추가로 우리가 select 하고자 하는 컬럼들에 우리가 받아오고 싶은 컬럼이 없다면 `sqlalchemy.sql.expression.func`를 이용 할 수 있습니다.
...
이 전의 예제에서는 `Select`가 두 테이블을 조인하고 자동으로  ON절을 생성할 수 있음을 확인했습니다. 하지만 이는 `user_table`, `address_table`에 ForeignKeyConstraint 정의가 포함되어 있어서 가능한 것이었습니다.
...
`Select.join()`과 `Select.join_from()`메서드 둘 다 키워드 인자로 `Select.join.isouter`,
`Select.join.full `를 받고 이를 각 각
