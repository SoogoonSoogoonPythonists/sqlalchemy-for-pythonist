...
객체 속성을 `Session.execute()`을 이용해 조회 할 경우에는 
...
`text()` 생성자는 이전 장 [3. 트랜잭션 및 데이터베이스API 작업](https://github.com/SoogoonSoogoonPythonists/sqlalchemy-for-pythonist/blob/main/tutorial/3.%20%ED%8A%B8%EB%9E%9C%EC%9E%AD%EC%85%98%20%EB%B0%8F%20DBAPI%20%EC%9E%91%EC%97%85.md)에서 한 번 소개가 되었는데요, 이 `text()`생성자안에  `SELECT`구문을 곧바로 넣어 사용할 수도 있었습니다.
